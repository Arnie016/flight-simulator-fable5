...
**Camera rail:** click the visible camera control, press **C/V**, or use gamepad **Y** to cycle **CHASE → COCKPIT → NOSE → TAIL → FREE → TOWER**. Cockpit mode exposes the integrated GLB dashboard and removes the duplicated external gauges; nose view hides the airframe for an unobstructed forward sightline, while chase and tail preserve the aircraft and external instrument strip.
...
Open `http://127.0.0.1:8643` and press **BEGIN FLIGHT**. That user gesture requests microphone permission, receives a short-lived client secret, and connects directly to `gpt-realtime-2` over WebRTC. The top voice control pauses or resumes microphone input; Settings retains manual connect/disconnect controls. The aviator can call the read-only `get_flight_state` tool for live phase, mission objective, speed, altitude, warnings, weather, and parked departure checks; it cannot change the flight controls.
...
Verify the six camera transforms, visible cycle order, integrated cockpit, unobstructed nose view, Realtime auto-start contract, voice pause state, and desktop/mobile toolbar fit:
...
**Realtime scope:** the Aviator bridge uses `gpt-realtime-2` with fixed `marin` voice output and one read-only flight-state tool. Civilian training is the active scope; multiplayer, dogfights, and fictional combat radio remain parked until authoritative networking exists.
