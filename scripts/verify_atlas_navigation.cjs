...
async function main(){
  ...
  const checks={
    ...
    authoredRegionsLegible:new Set([initial.state.labelLayout.focus.id,...initial.state.labelLayout.shown.map(label=>label.id)]).size===6&&initial.state.labelLayout.hidden.length===0,
